	...
	def run(self, arg, inter = True):
		...
		if len(arg) != 0:
			...
			if len(arg) > 0:  # if there is any another arguments function should be called separately
				for f in arg:
					#com = 'self.%s(\'%s\', inter = True)' % (func, f, '\'' + path + '\', ' if path != '' else '')
					print 'self.%s(\'%s\', inter = True)'
					print (func, f, '\'' + path + '\', ' if path != '' else '')
					if self.verbose:
						self.err.write(com + '\n')
					exec(com)
			else:
				com = 'self.%s(%sinter = True)' % (func, '\'' + path + '\', ' if path != '' else '')
				if self.verbose:
					self.err.write(com + '\n')
				exec(com)
