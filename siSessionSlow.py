...
class siSession(guanaco.webdav.webDAV):
	...
	def __init__(self, **args):
		...
		if not cont and len([i for i in xrange(len(cont)) if cont[i]['href']
			...
			self.upload(getJson(siSet), fromString = True)
			self.files = siSet()
		else:
			self.files = initFromJson(self.download('SaveItDir/files.si', toString = True))
		if not cont and len([i for i in xrange(len(cont)) if cont[i]['href']
				== '/SaveItDir/tags.si']) == 0:  # True if tags.si not exists
			self.upload(getJson(siSet), fromString = True)
			self.tags = siSet()
		else:
			self.tags = initFromJson(self.download('SaveItDir/tags.si', toString = True))
